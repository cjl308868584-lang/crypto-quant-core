...
216. [Nautilus Sandbox Preflight Comparison Schema](config/nautilus-sandbox-comparison-v1.schema.json)
217. [Nautilus Sandbox 隔离 Spike ADR-0063](docs/adr/0063-nautilus-sandbox-isolation-spike.md)
218. [实施追踪 v0.63.0](docs/implementation-status-v0.63.0.md)
219. [Replacement Challenger v2 冻结计划](artifacts/challenger-replacement/challenger-replacement-plan-v0.64.0.json)
220. [Replacement Challenger 存储安全 supersession ADR-0064](docs/adr/0064-replacement-challenger-plan-v2-storage-supersession.md)
221. [实施追踪 v0.64.0](docs/implementation-status-v0.64.0.md)
222. [NautilusTrader v0.65 预注册计划](artifacts/nautilus-sandbox/nautilus-e2e-spike-plan-v0.65.0.json)
...
227. [Replacement 三阶段事件 Runtime v0.66.0](docs/implementation-status-v0.66.0.md)
228. [Replacement Live Input / Deployment Candidate v0.67.0](docs/implementation-status-v0.67.0.md)
...
Git中的设计基线已冻结，当前代码版本为 `0.78.7`。从早期研究、Paper 调度与证据治理，到 replacement Challenger 的 DecisionOpportunity、公开模拟、Binance 私有适配、对账和分级 Canary 控制，均以版本化合同、严格 loader 与失败关闭测试绑定。当前里程碑仅为 `BINANCE_E0_CODE_COMPLETE_NOT_ACTIVATED`；完整验证仍必须提供 Artifact 之外的 trusted attestation hash，self-hash 不能自证来源可信。
...
v0.63冻结独立 Python 3.12 NautilusTrader 供应链边界以及 exact `1.227.0` wheel/tag/commit/license/传递依赖身份。官方 frozen environment 两次同源有界获取都未形成可用环境，且 exact terminal transcript 未作为可重放制品保留；因此报告诚实标记为 `SUPPLY_CHAIN_FETCH_NOT_MACHINE_REPLAYABLE / INCONCLUSIVE_BLOCKED`。本版本不保留未运行过的 fixture/request/result 协议，也没有 runner、BacktestEngine、Golden/replay 或 synthetic result。本结果不影响任何现有90天流，也不允许 Shadow 或采用。项目同时停止扩建通用交易引擎、Broker、交易所适配和通用 UI，把后续投入集中到证据可信、失败关闭、对账和长期 Paper 观察。

v0.64 在 replacement 未安装、未启动且未观察到 runtime root/plist/service/event
的前提下，用显式 supersession 把 v0.62 的 SQLite/artifact 存储路径合同更正为
append-only canonical event log 唯一权威。v0.62 bytes/tag 保持不变，研究范围、
决策、cohort、evidence、predecessor 失败和零交易权限保留；exports 仅是未来可重建
的非权威输出。机器 snapshot 只证明采集时的状态，“从未启动”历史事实由
owner attestation 承担，loader 不声称证明它为真。状态为
`PLAN_FROZEN_REPLACEMENT_V2_NOT_STARTED`：不含 runtime、deployment、start receipt 或任何
90 天证据。

v0.65 对 NautilusTrader `1.230.0` 完成单次、不可选择的端到端隔离 Spike ceremony。正式命令
由本机 CPython 3.9 启动，未满足预注册的 Python 3.12 平台门，因此在下载和 runner 前以
`NAUTILUS_V065_PLATFORM_MISMATCH / INCONCLUSIVE_KEEP_CURRENT_CORE` 失败关闭，
...
Binance Accounting Core v0.71.0 在 fixture-only、无凭据和无网络边界内增加
严格 Binance 输入、双向互斥 decision、signed Spot/perpetual accounting、fee、
funding、margin、equity 与 fail-closed risk transition。lifecycle not implemented：
订单/成交/UNKNOWN/stop/reconciliation 与 event integration 明确顺延 v0.72；本版
不安装、不启动、不创建生产状态，也不构成盈利、Paper、Canary 或实盘结论。

Replacement Readiness Integration v0.73.0 从 v0.72 的 Spot/永续完整生命周期
fixture 与 append-only DecisionOpportunity 事件中只读派生运行资格和严格 tail-blind
经济进度。起点必须绑定首个自然 OBSERVED；flat MISSED 只影响覆盖率，exposed MISSED
永久保留 gap lock。UNKNOWN、对账、止损、风险锁、边界和证据身份失败均失败关闭。
Operations Projection v2 与原 loopback-only 控制台仅显示机会、天数、周期、对账和
安全边界状态，不显示 PnL、收益率或提前经济结论。状态为
`READINESS_EVALUATOR_AND_READ_ONLY_INTEGRATION_VERIFIED_NOT_STARTED`：未安装、未启动、
未写 production root、未使用凭据/Broker/订单/资金，也没有开始 7 天或 90 天计时；
90 天最终经济阈值仍须未来单独预注册。

Replacement Economic Preregistration v0.74.0 现已冻结唯一的 90 天
DecisionOpportunity 经济评估计划：90 个固定资本日收益、固定七日 MBB、
全部样本和经济门、flat MISSED 乐观/悲观边界及不可选择的最终状态机。
它没有读取任何经济结果，也不授权 runtime、安装、启动、订单或资金操作。
最终经济评估器已发布但未运行；安装/启动仍是未来独立里程碑。

Accelerated Canary Supersession v0.75.0 仅为未来 activation 冻结新的运营资格：
最终连续 72 小时生产模拟和完整故障矩阵通过后，仍需独立批准最小金额的
`OPERATIONAL_CEREMONY_NOT_STRATEGY_EVIDENCE`，之后才可讨论 E0。v0.74 的
90 天经济总体、起点、尾部、指标和最终状态机保持不可变；仪式订单不进入策略周期
或经济证据。当前仍为 `CODE_COMPLETE_NOT_ACTIVATED_NOT_YET_REACHED`；
v0.77 Binance 私有边界/Canary bundle 尚未实现。

Public Simulation and Research Bundle v0.76.0 已实现无凭据公开行情捕获、
保守模拟、replacement-v3 event runtime/deployment/start loader、72小时运营资格、
冻结90天 evaluator、tail-blind 只读投影及一次36-case离线故障矩阵。状态为
`PUBLIC_SIMULATION_AND_RESEARCH_CODE_RELEASED_NOT_ACTIVATED`；它没有安装或启动，
没有读取账户或经济结果，也没有开始72小时或90天计时。

Binance Private Canary Bundle v0.77.0 已完成 Binance-only ETHUSDT 的
Spot-long / USDⓈ-M perpetual-short 私有适配器、严格账户预检、订单/成交/
UNKNOWN/保护止损/费用/funding/仓位/余额对账、E0/E1/E2 控制器、只读运维扩展、
disabled 配置模板、runbooks 及 59-case 离线故障证据。当前唯一结论为
`CODE_COMPLETE_NOT_ACTIVATED`：没有安装或启动服务，没有创建或读取凭据，
没有访问真实 Binance 私有接口、提交订单或移动资金，也没有开始72小时或90天计时。

Replacement v3 Simulation Activation Trust Chain v0.78.7 以 release-scoped candidate
...
v0.78.5 的部分安装证据保持原样；v0.78.6 修复 install receipt 的 canonical
`.000Z` 时间，v0.78.7 新增严格只读 qualification、不可变 recovery receipt 和
release-scoped 新 target。发布本身 does not authorize installation or start；旧证据
must not be deleted，服务保持 disabled、unloaded。
详细完成度见
[实施追踪 v0.78.7](docs/implementation-status-v0.78.7.md)，启动步骤见
...
[ADR-0082](docs/adr/0082-v0784-preflight-hotfix.md)。
...
- 不超过冻结的 2x 技术硬上限；E0 仅允许 0.5x 毛敞口，E1 仅允许 1x，E2 也必须通过独立晋级门。
