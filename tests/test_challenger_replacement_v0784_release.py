...
class V0784ReleaseTests(unittest.TestCase):
    def test_patch_version_manifest_and_activation_binding_are_exact(self):
        ...
        self.assertIn('"tag": "v0.78.4"', trust)
        self.assertIn('manifest["manifest_version"] != "1.76.0"', trust)
        self.assertIn('"git", "rev-parse", "v0.78.4^{}"', preflight)
