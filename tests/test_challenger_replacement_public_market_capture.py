...
COMMITTED_CAPTURE = Path(__file__).parent / "fixtures" / (
...
def _payloads():
    ...
    return (
        ...
        ("funding_history", FUNDING_URL, 1024 * 1024, [{
            "symbol": "ETHUSDT", "fundingTime": 1787716800000, "fundingRate": "-0.0001", "markPrice": "3310.25", "fundingRateType": "REGULAR",
        }]),
    )
...
def _outer_document():
    ...
    document = {
        ...
        "schema_version": "2.0.0", "capture_id": "", "capture_hash": "0" * 64,
        ...
        "normalized": {
            ...
            "funding_records": [{"funding_time": SCHEDULED_FOR, "rate": "-0.0001", "mark": "3310.25"}],
            "simulation_rules": {
...
class PublicMarketCaptureLoaderTests(unittest.TestCase):
    ...
    def test_funding_interval_order_type_and_limit_are_rejected(self):
        ...
        special = _outer_document()
        funding = _request_payload(special, 5)
        funding[0]["fundingRateType"] = "SPECIAL"
        _replace_request_payload(special, 5, funding)
        ...
        for document in (outside, special, duplicate, overflow):
    ...
    def test_committed_public_market_capture_replays_to_frozen_identity(self):
        body = COMMITTED_CAPTURE.read_bytes()
