"""CLI for fixed-order Challenger cohort evidence maintenance."""
...
from .challenger_cohort_evidence_maintenance import (
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="challenger-cohort-evidence-maintain"
    )
    ...
    *,
    ...
    try:
        ...
        raise ChallengerCohortEvidenceMaintenanceError(
            "CHALLENGER_COHORT_EVIDENCE_ROOT_INVALID"
        ) from error
        ...
        for index, left in enumerate(roots):
            for right in roots[index + 1 :]:
                if (
                    ...
                    raise ChallengerCohortEvidenceMaintenanceError(
                        "CHALLENGER_COHORT_EVIDENCE_ROOT_OVERLAP"
                    )
        observed_at = (
            clock() if clock is not None else datetime.now(timezone.utc)
        )
        summary = maintain_challenger_cohort_evidence(
