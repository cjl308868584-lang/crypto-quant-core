"""Offline CLI for all-inclusive Challenger cohort economic results."""
...
from .challenger_cohort_daily_archive import ChallengerCohortDailyArchiveError
from .challenger_cohort_economic_results import (
    ChallengerCohortEconomicResultError,
    publish_all_cohort_economic_results,
)
from .challenger_cohort_episode_receipt import ChallengerCohortEpisodeReceiptError
...
class ChallengerCohortEconomicResultCLIError(ValueError):
    """A CLI path or invocation failed closed."""


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="challenger-cohort-economic-result"
    )
...
def _trusted_root(
    ...
    *,
    ...
    try:
        ...
        raise ChallengerCohortEconomicResultCLIError(
            "CHALLENGER_COHORT_ECONOMIC_ROOT_INVALID"
        ) from error
...
def main(
    ...
    *,
    ...
    try:
        ...
        roots = (receipt_root, archive_root, result_root)
        for index, left in enumerate(roots):
            for right in roots[index + 1 :]:
                if (
                    ...
                    raise ChallengerCohortEconomicResultCLIError(
                        "CHALLENGER_COHORT_ECONOMIC_ROOT_OVERLAP"
                    )
        summary = (
            publisher or publish_all_cohort_economic_results
        )(
        ...
        ChallengerCohortEconomicResultCLIError,
        ChallengerCohortEconomicResultError,
